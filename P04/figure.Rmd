...
Yuta Suzuki, Takuya Fuji, Tetsuto Minami, Shigeki Nakauchi
...
if(exists(".anovakun.env")){
  sys.source("anovakun_485.R", envir = .anovakun.env)
}else{
...
data=fromJSON(file="./data_e1.json")
...
#### making dataset for e1 #################################
# dat <- readMat("./data_exp2.mat")
# lut <- readMat("./lut_desplay++.mat")
dat = fromJSON(file="./data_e2.json")
lut = unlist(fromJSON(file="./data_disp.json"))
# dat <- list((matrix(unlist(data$PLR),nrow=length(data$PLR),byrow=T)),
...
#### making dataset for e1 #################################
data=fromJSON(file="./data_e3.json")
dat <- list(matrix(unlist(data$y),nrow=length(data$y),byrow=T),
            t(unlist(data$sub)),t(unlist(data$condition)),t(unlist(data$taskRes)))
names(dat) <- c('y', 'sub', 'condition', 'taskRes')
