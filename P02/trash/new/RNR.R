...
source( "/Users/Yuta-PC/Documents/matlab/Rscript/clearFunc.R" )
clearAll()
...
for (i in 1:240) {
  ...
  m1 <- lmer(PLR ~ condition + (1|subject),data=s_datatmp)
  t <- summary(m1)$coefficients[,"Pr(>|t|)"]
  t <- as.numeric(t)
...
if( dev.cur() > 1 ) dev.off()
  ...
  # par(new=T)
  plot(xx,allListCondition,type="l",col=i, xlim=c(startTime,endTime), ylim=c(0, 0.1),ylab="p-Value", lty=1)
  par(new=T)
legend("topleft",
       legend=c("1", "2"),
       lty=c(1,1),
